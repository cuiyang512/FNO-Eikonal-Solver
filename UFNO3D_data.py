...
class U_net(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('x:', x.shape, 'out_conv1:', out_conv1.shape)
        out_conv2 = self.conv2_1(self.conv2(out_conv1))
        out_conv3 = self.conv3_1(self.conv3(out_conv2))
        out_deconv2 = self.deconv2(out_conv3)
        # print('out_conv2:', out_conv2.shape, 'out_deconv2:', out_deconv2.shape)
        out_deconv2 = out_deconv2[:, :, :out_conv2.shape[2], :out_conv2.shape[3], :out_conv2.shape[4]]  # Crop to match
        concat2 = torch.cat((out_conv2, out_deconv2), 1)
        out_deconv1 = self.deconv1(concat2)
        out_deconv1 = out_deconv1[:, :, :out_conv1.shape[2], :out_conv1.shape[3], :out_conv1.shape[4]]  # Crop for concat1
        concat1 = torch.cat((out_conv1, out_deconv1), 1)
        out_deconv0 = self.deconv0(concat1)
        out_deconv0 = out_deconv0[:, :, :x.shape[2], :x.shape[3], :x.shape[4]]  # Crop for concat0
